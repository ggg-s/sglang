...
The value is a positive integer in MiB. NVIDIA defaults to 512 MiB. This is
a port of `sglang-sxf`'s `codex/hicache-optimization` through `95e8253f1` to
the older `feat/dsv4-pdmux-opt` base. This base has no AMD FP4 indexer backend,
so the source branch's AMD FP4 pool integration is not applicable here. The
shared budget helper retains the legacy setting's parsing for compatibility.
...
indexer path do not acquire the new budget-based split. SM120 row-limit
splitting is ported as a prerequisite, including matching chunk-local TopK plans.
