...
class TestChunkMetadata(unittest.TestCase):
    def setUp(self):
        ...
        envs = types.SimpleNamespace(
            SGLANG_OPT_USE_AITER_INDEXER=types.SimpleNamespace(get=lambda: False),
    ...
    def make(self, rows=2050, *, prefill=True, graph=False, value=1):
        return self.metadata.PagedIndexerMetadata(
            ...
            c4_seq_lens=Tensor(np.full(rows, value, dtype=np.int32)),
            use_topk_v2=True,
            is_prefill=prefill,
    ...
    def test_tail_and_matching_topk_plans(self):
        ...
        np.testing.assert_array_equal(
            np.concatenate(self.dg_calls).reshape(-1), m.c4_seq_lens.value
        )
    ...
    def test_runtime_padding_and_cropping_rebuild_chunk_plans(self):
        ...
        for target_rows in (2050, 3073, 1025, 1024):
            with self.subTest(target_rows=target_rows):
                ...
                ns = dict(
                    envs=types.SimpleNamespace(
                        SGLANG_TOPK_TRANSFORM_512_TORCH=types.SimpleNamespace(
                            get=lambda: False
                        )
                    ),
                ...
                self.assertEqual(actual.c4_seq_lens.shape[0], target_rows)
                self.assertEqual(actual.page_table.shape[0], target_rows)
                self.assertEqual(original.c4_seq_lens.shape[0], 2050)
                if target_rows == 2050:
                    self.assertIs(actual, original)
                else:
                    self.assertIsNot(actual, original)
                if target_rows > 2050:
                    np.testing.assert_array_equal(actual.c4_seq_lens.value[2050:], 1)
                ...
                if target_rows <= 1024:
                    ...
                    for idx, start in enumerate(range(0, target_rows, 1024)):
                        length_sum = int(
                            actual.c4_seq_lens.value[start : start + 1024].sum()
                        )
                        self.assertEqual(
    ...
    def test_alignment_cache_is_per_forward_and_per_row_count(self):
        ...
        np.testing.assert_array_equal(rebuilt.c4_seq_lens.value[:2050], 3)
        np.testing.assert_array_equal(rebuilt.c4_seq_lens.value[2050:], 1)
    ...
    def test_indexer_dispatch_matches_whole_topk_and_reuses_plans(self):
        ...
        ns = dict(
            envs=types.SimpleNamespace(
                SGLANG_TOPK_TRANSFORM_512_TORCH=types.SimpleNamespace(get=lambda: False)
            ),
            torch=types.SimpleNamespace(Tensor=Tensor),
            ...
            _c4sl=m.c4_seq_lens.unsqueeze(-1),
            c4_seq_lens=m.c4_seq_lens,
        ...
        # This older base also supports raw-index capture and its original
        # Torch/FlashInfer TopK entry points. Verify every chunk writes to the
        # matching output rows on these fallback paths too.
        def reduce_raw(scores, lens, pages, out, page_size, raw):
            selected = np.argsort(-scores.value, axis=1)[:, :4]
            out.value[...] = selected
            if raw is not None:
                raw.value[...] = selected

        for backend in ("v1", "capture", "torch", "flashinfer"):
            with self.subTest(backend=backend):
                output.value.fill(-1)
                raw = Tensor(np.full((rows, 4), -1))
                m.use_topk_v2 = backend == "capture"
                ns["raw_indices"] = raw
                ns["self"].dsa_topk_backend = types.SimpleNamespace(
                    is_torch=lambda: backend == "torch",
                    is_flashinfer=lambda: backend == "flashinfer",
                )
                ns.update(
                    topk_transform_512=reduce_raw,
                    topk_transform_512_pytorch_vectorized=reduce_raw,
                    topk_transform_512_flashinfer_unfused=reduce_raw,
                )
                exec(compiled, ns)
                selected = np.argsort(-expected, axis=1)[:, :4]
                np.testing.assert_array_equal(output.value, selected)
                np.testing.assert_array_equal(raw.value, selected)
