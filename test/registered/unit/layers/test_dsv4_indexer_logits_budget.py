    ...
    def test_invalid_budgets(self):
        for value in ("0", "-1", "bad", "1.5"):
            with (
                ...
                patch.dict(os.environ, {"SGLANG_DSV4_INDEXER_LOGITS_BUDGET_MB": value}),
            ):
...
class TestChunkMetadata(unittest.TestCase):
    ...
    def test_indexer_dispatch_matches_whole_topk_and_reuses_plans(self):
        ...
        def reduce(scores, lens, pages, out, page_size, plan, raw, enable_cluster):
        ...
        # The newer main supports raw-index output directly in TopK v2.
        # Chunk views must write every row of both destination buffers.
        raw = Tensor(np.full((rows, 4), -1, dtype=np.int64))
        ns["raw_indices"] = raw
        output.value.fill(-1)
        exec(compiled, ns)
        np.testing.assert_array_equal(raw.value, np.argsort(-expected, axis=1)[:, :4])
        np.testing.assert_array_equal(output.value, raw.value)
        self.assertEqual(plan_count, (len(self.dg_calls), len(self.topk_calls)))
